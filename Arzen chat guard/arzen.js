...
client.on("ready", () => {
    client.user.setPresence({ activity: { name: "Arzen is here?" }, status: "dnd" })
    client.channels.cache.get(config.ses).join()
})

client.login(config.token).then(() => console.log(`${client.user.username} Olarak Giriş Yapıldı Bot Aktif Arzen was here`)).catch(() => console.log("Bot Giriş Yaparken Bir Hata Oldu"))
...
client.on('message', async message => {
    ...
    if (reklamlar.some(word => message.content.toLowerCase().includes(word))) {
        ...
        if (db.fetch(`reklam_${message.author.id}`) === 1)  {
            message.channel.send(`${message.author} Reklam yapmana izin vermem! Eğer 2 kez daha reklam yaparsan seni banlayacağım! (1/3)`).then(arzenxd => arzenxd.delete({ timeout: 5000 }))
        }
        else if (db.fetch(`reklam_${message.author.id}`) === 2) {
            message.channel.send(`${message.author} Reklam yapmana izin vermem! Eğer 1 kez daha reklam yaparsan seni banlayacağım! (2/3)`).then(arzenxd => arzenxd.delete({ timeout: 5000 }))
        }
            ...
            message.guild.members.ban(message.author.id, {reason: "Arzen Chat Guard"}).catch(err => message.channel.send(`${message.author} Kişisini banlayamadım`))
    ...
    if (kufur.some(word => msg.content.includes(word))) {
        try {
            if (!msg.member.permissions.has("ADMINISTRATOR")) {
                ...
                return msg.reply('Heey! Küfür Yasak.').then(arzenxd => arzenxd.delete({ timeout: 5000 }))
            }
        } catch (err) {
            console.log(err);
        }
        ...
        if (msg.content == caps) {
          if (!msg.member.permissions.has("ADMINISTRATOR")) {
            if (!msg.mentions.users.first()) {
              msg.delete();
              return msg.channel.send(`${msg.member}, Capslock Kapat Lütfen!`).then(arzenxd => arzenxd.delete({timeout: 5000}))
    ...
    if (kufur.some(word => msg.content.includes(word))) {
        try {
            if (!msg.member.permissions.has("ADMINISTRATOR")) {
                ...
                return msg.reply('Yakaladım Seni! Küfür Yasak.').then(arzenxd => arzenxd.delete({ timeout: 5000 }))
            }
        } catch (err) {
            console.log(err);
        }
    ...
    if (map.has(message.author.id)) {
        ...
        if (difference > DIFF) {
            ...
            if (parseInt(msgCount) === lımıt) {
                ...
                message.channel.send("Spam  yaptığından dolayı 15 dakika boyunca susturuldun.").then(arzenxd => arzenxd.delete({ timeout: 5000 }))

                setTimeout(() => {
                    if (!message.member.roles.cache.get(config.mutedrole)) return;
                    message.member.roles.remove(mutedrole);
                    message.channel.send("Muten açıldı lütfen tekrar spam yapma.").then(arzenyxd => arzenxd.delete({ timeout: 5000 }))
                }, 900000);//9000000
            } else {
